...
CREATE TABLE `system_organize` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `organize_code` varchar(50) NOT NULL DEFAULT '' COMMENT '部门编码 001_001',
  `organize_level` bigint(10) unsigned DEFAULT '1' COMMENT '部门级别 默认1 最多10',
  `organize_addr` varchar(32) DEFAULT NULL COMMENT '部门地址',
  `organize_connect` varchar(16) DEFAULT NULL COMMENT '部门联系电话',
  `organize_num` varchar(16) DEFAULT NULL COMMENT '部门识别码',
  `organize_per_code` varchar(50) NOT NULL DEFAULT '' COMMENT '上级部门编码 001',
  `desc` varchar(255) DEFAULT '' COMMENT '备注说明',
  `status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '状态(0:禁用,1:启用)',
  `authorize` varchar(255) DEFAULT NULL,
  `is_deleted` tinyint(1) unsigned DEFAULT '0' COMMENT '删除状态(1:删除,0:未删)',
  ...
  `create_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `index_system_organize_username` (`organize_code`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=10145 DEFAULT CHARSET=utf8 COMMENT='系统组织表';